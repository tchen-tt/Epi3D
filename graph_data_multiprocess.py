    ...
    def __call__(self, interval: Interval, shuffle: bool = False) -> Any:
        ...
        with torch.no_grad():
            ...
            try:
                ...
                print(f"{str(interval)}")
                print(enformer_interval_list)
                print(f"Error extracting enformer output: {e}")
                raise RuntimeError(f"Error extracting enformer output: {e}")
                return None
...
class CreateGraphData(EpiConcatMatrix):
    ...
    def __call__(self) -> List[Data]:
        ...
        # Use single-threaded processing for now
        # Multiprocessing can be added later if needed
...
if __name__ == "__main__":
    ...
    datast = GraphDataset(root=f"./develop_test/graph_data_4096_2M", data_list=outputs)
